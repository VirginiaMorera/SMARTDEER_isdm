...
all_data_sp <- all_data_sf %>% 
  st_transform(st_crs("+proj=tmerc +lat_0=53.5 +lon_0=-8 +k=0.99982 +x_0=600000 +y_0=750000 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")) %>% 
  as_Spatial()
...
mesh <- inla.mesh.2d(boundary = list(boundary_in, boundary_out), 
                     ...
                     cutoff = 10000, crs = boundary_sp@proj4string)
```
...
meshLarge <- inla.mesh.2d(boundary = list(boundary_in, boundary_out), 
                          ...
                          cutoff = 20000, crs = boundary_sp@proj4string)
```
