...
sel_data <- all_data %>% 
  dplyr::select(County, Site_id, Year, Species, Deer.Presence, Deer.Count, 
                Y = Latitude, X = Longitude, Source, Type) %>% 
  filter(X > 421874) %>% 
...
PA_data <- sel_data %>% 
  ...
  group_by(Species, Site_id, Source, Type, X, Y) %>% 
  summarise(count_any = sum(PA)) %>% 
  ungroup() %>% 
  mutate(Deer.Presence = if_else(count_any == 0, 0, 1)) %>% 
  dplyr::select(-count_any)
...
PA_covars <- PA_covars %>% 
  mutate(landCover2 = recode(landCover, "1" = "Built area", 
                             "2" = "Saltwater related", "3" = "No vegetation", 
                             "4" = "Freshwater related", "5" = "Other vegetation", "6" = "Agricultural land", 
                             "7" = "Pastures", "8" = "Grassland", 
                             "9" = "Transitional", "10" = "Coniferous forest", 
                             "11" = "Mixed forest", "12" = "Broadleaf forest")) %>%
  mutate(landCover2 = factor(landCover2, levels = c("Built area", 
                                                    "Saltwater related", 
                                                    "No vegetation", 
                                                    "Freshwater related", 
                                                    "Other vegetation", 
...
PO_ppp <- ppp(x = st_coordinates(PO_data_sf)[,1], 
              y = st_coordinates(PO_data_sf)[,2], 
              window = PO_win)
...
RL <- list(env_data$landCover, env_data$tree_cover_density, 
           env_data$elevation, env_data$slope, env_data$human_footprint_index, 
           PO_raster2)
# devtools::source_url("https://github.com/VirginiaMorera/Useful-little-functions/blob/master/list_to_stack.R?raw=TRUE")

new_stack <- list_to_stack(RL, new_res = raster::res(env_data), 
                           dest_crs = env_data@crs, turn_0_to_NA = T)

plot(new_stack)
names(new_stack)[6] <- "Fallow_deer_per_cell"
cor <-layerStats(new_stack, 'pearson', na.rm = T)
ggcorrplot(cor$`pearson correlation coefficient`, method = "circle", 
           show.diag = F, type = "upper", lab = T) 
